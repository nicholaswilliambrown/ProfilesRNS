...
function addDimensionsBadgesAndCheckLabel() {
    $.when(window.__dimensions_embed.addBadges()).done(function () {
        setTimeout(function () {
            ...
            $(".__dimensions_Badge_Image").prepend(`<img alt="dimensions citations" class="__cits__" src=${gPerson.citationsPngUrl}>`);
...
function addPmcAndRcrCitations(linkItems, pub) {
    ...
    if (pmcCitations) {
        ...
        graphic.css("background-image", `url("${gPerson.pmcImageUrl}")`);
