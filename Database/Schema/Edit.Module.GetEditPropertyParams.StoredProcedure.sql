...
BEGIN
	...
	select @propertyURI = isnull(a.Property, 'ERROR'), @propertyName = isnull(a._PropertyLabel, 'Unknown Property'), @editModule = isnull(EditModule, 'Edit.Error'), @maxCardinality = isnull(MaxCardinality, 0), @viewSecurityGroup =isnull(a.ViewSecurityGroup, Subject) from [Ontology.].ClassProperty a 
		join [RDF.].Triple b
	...
	select (select @propertyURI propertyURI, @propertyNode propertyName, @editModule propertyModule, @maxCardinality maxCardinality, @viewSecurityGroup viewSecurityGroup for json path , WITHOUT_ARRAY_WRAPPER) as editPropertyParams
END
GO
