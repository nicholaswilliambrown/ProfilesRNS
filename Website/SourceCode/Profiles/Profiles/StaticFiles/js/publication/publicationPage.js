async function setupPublicationPage() {
    ...
    let topData = findModuleDataByName(json, 'Publication.GeneralInfo');
    let subjectsData = findModuleDataByName(json, 'Publication.Concepts');
    let authorsData = findModuleDataByName(json, 'Publication.Authors');
...
function emitPublicationHelper(data, mainDiv, subTitle, displayProperty) {
    ...
    for (let i=0; i<data.length; i++) {
        ...
        let anchor = createAnchorElement(item[displayProperty], item.URL);
