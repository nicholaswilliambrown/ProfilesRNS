...
// cw: see https://stackoverflow.com/questions/58429853/this-site-overrides-array-from-errors-on-magento-using-prototypejs
...
if (Prototype.BrowserFeatures.XPath) {
  document._getElementsByXPath = function(expression, parentElement) {
    var results = [];
    var query = document.evaluate(expression, $s(parentElement) || document,
...
Element.Methods = {
  ...
  identify: function(element) {
    ...
    if (id) return id;
    do { id = 'anonymous_element_' + Element.idCounter++ } while ($s(id));
  ...
  hasClassName: function(element, className) {
    ...
    return (elementClassName.length > 0 && (elementClassName == className ||
      new RegExp("(^|\\s)" + className + "(\\s|$s)").test(elementClassName)));
  },
  ...
  removeClassName: function(element, className) {
    ...
    element.className = element.className.replace(
      new RegExp("(^|\\s+)" + className + "(\\s+|$s)"), ' ').strip();
    return element;
  },
  ...
  getOpacity: function(element) {
    return $s(element).getStyle('opacity');
  },
...
(function() {
  ...
  function getOffsetParent(element) {
    ...
    if (!isInline && element.offsetParent) return $s(element.offsetParent);
...
var Expr = Sizzle.selectors = {
	...
	match: {
		...
		POS: /:(nth|eq|gt|lt|first|last|even|odd)(?:\((\d*)\))?(?=[^-]|$s)/,
		PSEUDO: /:((?:[\w\u00c0-\uFFFF-]|\\.)+)(?:\((['"]*)((?:\([^\)]+\)|[^\2\(\)]*)+)\2\))?/
	},
	leftMatch: {},
	attrMap: {
